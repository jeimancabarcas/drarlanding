  ...
  template: `
    <section id="formacion" class="py-24 bg-slate-50 relative overflow-hidden">
      ...
      <div class="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        <div class="text-center mb-16 header-anim opacity-0"> <!-- Initial opacity 0 class to prevent FOUC -->
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 cert-grid">
           <!-- Card 1 -->
           <div class="cert-card opacity-0 bg-white p-8 rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-blue-200 group">
...
export class CertificationsComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    this.ctx = gsap.context(() => {
      // Changed to .to() instead of .from() to avoid state conflicts
      // We set initial opacity-0 in HTML class to ensure they are hidden at start
      
      gsap.to('.header-anim', {
        scrollTrigger: { 
            trigger: '.header-anim', 
            start: 'top 95%', // Trigger almost immediately
      ...
      // Setup initial state for animation target if not handled by CSS class correctly
      gsap.set('.cert-card', { y: 40, opacity: 0 });
      ...
      gsap.to('.cert-card', {
        ...
        stagger: 0.15,
        ease: 'power2.out',
        clearProps: 'transform' // Clear transform after animation to avoid blurriness
      });
    }, this.el);
  }
